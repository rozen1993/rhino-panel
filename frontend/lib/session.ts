...
export async function currentRole(): Promise<Role | null> {
  ...
  const cookieAccounts = parseAccountsCookie(store.get(accountsCookieName)?.value);
  const seed = defaultAccounts.find((item) => item.id === accountId);
  const account = cookieAccounts.find((item) => item.i === accountId) ?? (seed ? { i: seed.id, u: seed.username, p: seed.password, r: seed.roleId, b: seed.bursonLinked, a: seed.active, n: seed.name } : undefined);
  if (!account?.a || account.r !== roleId) return null;
  return { ...roles[roleId], accountId, accountName: account.n, bursonLinked: account.b };
}
