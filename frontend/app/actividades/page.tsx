  ...
  return (
    <MobileShell coordination={coordination} initials={coordination ? "CH" : "JV"} user={coordination ? "Chiara" : "Johann"}>
      <main className="space-y-4 px-3 py-4">
        ...
        <section aria-label="Resumen mensual" className="grid grid-cols-2 gap-2">
        ...
        <section aria-labelledby="activity-list-title" className="space-y-2">
          ...
          {activities.map((activity) => <ActivityCard activity={activity} key={activity.id} showResponsible={coordination} />)}
        </section>

        <Link className="sticky bottom-[4.75rem] flex min-h-12 w-full items-center justify-center rounded-[5px] border border-amber bg-amber px-5 py-2 text-sm font-bold text-ink" href="/actividades/nueva">＋ Nueva actividad</Link>
      </main>
    </MobileShell>
  );
}
