...
export default async function NewActivityPage({ searchParams }: PageProps<"/actividades/nueva">) {
  ...
  return <MobileShell backHref={activityId ? `/actividades/${activityId}` : "/actividades"} role={role}>
    <main className="mx-auto max-w-5xl space-y-4 px-3 py-4 md:px-6 md:py-6">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-line pb-4">
        ...
        <span className="rounded-full border border-cyan/30 bg-cyan/10 px-3 py-1.5 text-xs font-extrabold text-[#08718a]">FLUJO OPERARIO</span>
      </header>
