...
export function DetailPanel({ item, choices, onChoose, close, titleId, closeButtonRef, today, selectedDate, clientView = false }: {
  item: DetailActivity;
  choices: DetailActivity[];
  onChoose: (item: DetailActivity) => void;
  close?: () => void;
  titleId: string;
    ...
    <aside aria-labelledby={titleId} ref={panelRef}
      ...
      {listing ? (
        ...
        <div className="space-y-5 p-4" key={item.id}>
          ...
          {!clientView && <div className="flex items-center gap-3 rounded-[8px] bg-panel-secondary p-3">
          ...
          {!clientView && <div className="divide-y divide-line/30 border-y border-line/30">
            <details>
