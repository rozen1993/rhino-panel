...
export function TopBar({ name, initials, backHref, role }: { name: string; initials: string; backHref?: string; role?: Role }) {
  return (
    <header className="flex min-h-16 items-center justify-between border-b border-line bg-panel px-4 sm:px-7">
      ...
      <div className="flex items-center gap-2">{role && <RoleSwitcher role={role} />}<Avatar initials={initials} size="sm" /><span className="hidden text-sm font-semibold sm:inline">{name}</span></div>
    </header>
  );
}
