...
export function FormField({ id, label, required = false, className = "", ...props }: FormFieldProps) {
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      <input className="min-h-11 rounded-md border border-line bg-panel px-3 py-2 text-sm text-ink placeholder:text-ink-muted focus:border-blue" id={id} required={required} {...props} />
    </div>
  );
}
