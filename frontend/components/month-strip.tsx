...
export function MonthStrip({ counts, activeMonth, onSelect, year = 2026 }: Props) {
  ...
  return <nav aria-label="Navegación mensual" className="panel-glow overflow-hidden rounded-[10px] border border-line bg-panel">
    ...
    <MonthWindow activeMonth={activeMonth} className="hidden md:grid xl:hidden" counts={counts} focusMonth={focusMonth} onFocus={setFocusMonth} onSelect={onSelect} size={4} year={year} />
    <MonthWindow activeMonth={activeMonth} className="hidden xl:grid" counts={counts} focusMonth={focusMonth} onFocus={setFocusMonth} onSelect={onSelect} size={6} year={year} />
  </nav>;
}

function MonthWindow({ activeMonth, className, counts, focusMonth, onFocus, onSelect, size, year }: Props & { className: string; focusMonth: number; onFocus: (month: number) => void; size: number }) {
  ...
  return <ol aria-label="Actividades por mes" className={`${className} ${columns} divide-x divide-line`}>
    ...
    {visible.map((month) => {
      ...
      return <li className="flex" key={month}><button aria-current={active ? "date" : undefined} aria-label={`Mostrar ${month.toLowerCase()} de ${year}: ${counts[monthIndex]} actividades`} className={`flex min-h-[4.15rem] w-full flex-col items-center justify-center px-2 text-center transition-colors ${active ? "action-surface text-[#173000]" : "text-ink hover:bg-panel-secondary"}`} onClick={() => { onSelect(monthIndex); onFocus(monthIndex); }} type="button">
