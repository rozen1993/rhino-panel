  ...
  return (
    <form className="space-y-4" onChange={markDraft} onSubmit={(event) => event.preventDefault()}>
      ...
      <Card className="space-y-4 p-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue">Obligatorios</p><p className="mt-1 text-xs text-ink-muted">Completa primero lo necesario para programar el trabajo.</p></div>
        ...
        <FormField defaultValue={editing ? "Cobertura de mantenimiento en peaje Chillón" : undefined} id="activity-title" label="Título" placeholder="Escribe el título de la actividad" required />
      ...
      <Card className="space-y-3 p-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue">Ubicación</p><p className="mt-1 text-xs text-ink-muted">El nombre del lugar basta; añade precisión solo si hace falta.</p></div>
        ...
        <details className="rounded-[5px] border border-line bg-panel-secondary p-3">
      ...
      <Card className="space-y-4 p-4">
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-blue">Detalles y entrega</p>
        <TextAreaField id="activity-description" label="Descripción" placeholder="Describe la actividad" />
        ...
        <p className="-mt-2 text-xs text-ink-muted">Enlace a la carpeta del material.</p>
        <TextAreaField id="activity-notes" label="Notas" placeholder="Notas internas para el equipo" />
      </Card>

      <Button className="w-full" type="submit">{editing ? "Guardar cambios" : "Guardar actividad"}</Button>
    </form>
  );
}
