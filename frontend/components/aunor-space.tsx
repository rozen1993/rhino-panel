...
import { aunorCode, aunorDisclaimer, replacementsForService, type AunorActivityRow, type AunorWorkspace, type AunorCommand, type AunorReplacement } from "@/lib/aunor";
import type { Role } from "@/lib/roles";
import type { Json } from "@/lib/supabase/database.types";
import { safeMaterialUrl } from "@/lib/external-link";
import s from "./aunor-space.module.css";

export type AunorScene="panel"|"detail"|"acordado"|"calendar"|"replacement";
...
export function AunorActivityCard({a,w}:{a:AunorActivityRow;w:AunorWorkspace}) {
  ...
  return <article className={s.card}>
    ...
    <div className={s.footer}><ServiceTag a={a} w={w}/></div>
...
function ActivityDetail({w,id,mutate,pending}:{w:AunorWorkspace;id:string;mutate:Mutation;pending:boolean}) {
  const a=w.activities.find(a=>a.id===id);
...
function Agreed({w}:{w:AunorWorkspace}) {
  ...
  return <><div className={s.notice}><strong>Una lista sencilla, sin porcentajes de cumplimiento</strong><p className={s.muted}>Referencia: cláusula 2.2 del contrato aportado. Las cantidades y equivalencias no se resuelven en esta vista. “Observado” indica un reemplazo documentado, no un incumplimiento. La marca permanece tras su confirmación.</p></div><div className={s.grid}>
    <section className={s.card+" "+s.pad}><p className="data-label text-cyan-ink">Lista simplificada de servicios</p><h2 className="section-title">Qué está previsto</h2>{w.services.map(service=><button className={s.service} key={service.id} type="button" aria-pressed={selected===service.id} onClick={()=>setSelected(service.id)}><strong>{service.label}</strong>{replacementsForService(w,service.id).length>0&&<span className={s.observed}><span aria-hidden="true" className={s.observedIcon}>!</span><span>Observado: tiene reemplazo</span></span>}<small>Referencia contractual: {service.reference} · Ver trabajos relacionados →</small></button>)}<p className={s.footnote}>Nombres abreviados para lectura. Esta lista no modifica el texto firmado.</p></section>
    <div className={s.stack}><section className={s.card+" "+s.pad}><p className="data-label text-cyan-ink">Servicio seleccionado · 2.2</p><h2 className="section-title">{serviceName(w,selected)}</h2>{replacements.length>0&&<p className={s.observed}><span aria-hidden="true" className={s.observedIcon}>!</span><span>Observado: tiene reemplazo</span></p>}{!related.length&&<p className={s.muted}>Todavía no hay trabajos relacionados con este servicio.</p>}{related.map(a=><div className={s.box+" "+s.sectionGap} key={a.id}><strong>{a.title}</strong><p>{a.not_performed_reason?"No se realizó: "+a.not_performed_reason:a.status==="Entregada"?"Trabajo realizado. La confirmación de su entrega es independiente.":a.status}</p><Link className={s.link} href={activityHref(a.id)}>Ver actividad →</Link></div>)}{replacements.map(r=><div className={s.sectionGap} key={r.id}><ReplacementSummary r={r} w={w}/><Link className={s.btn+" "+s.sectionGap} href={replacementHref(r.id)}>Ver original, sustituto y evidencia →</Link></div>)}</section>
...
function PublicCalendarDetail({w,item,titleId,close,closeButtonRef,choices,onChoose}:{w:AunorWorkspace}&CalendarDetailProps) {
  ...
  return <aside className={s.calendarDetail} aria-labelledby={titleId}>{close&&<button className={s.close} type="button" aria-label="Cerrar detalle" ref={closeButtonRef} onClick={close}>×</button>}
    ...
    <StatusPill status={a.status}/><p className="data-label mt-5 text-cyan-ink">{a.type}</p><h2 className="section-title" id={titleId}>{a.title}</h2><AunorJourneys w={w} id={a.id}/><p className={s.state}>{a.summary}</p><div className={s.sectionGap}><ServiceTag w={w} a={a}/></div><Link className={s.btn+" "+s.primary+" "+s.sectionGap} href={activityHref(a.id)}>Ver actividad y entrega →</Link>
  </aside>;
}
export function AunorSpace({initial,scene,id="",demo=false,year=2026,today="2026-09-06"}:{initial:AunorWorkspace;role:Role;scene:AunorScene;id?:string;demo?:boolean;year?:number;today?:string}) {
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      ...
      catch{if(active)setError("No se pudo actualizar la información. Reintenta cuando vuelva la conexión.");}
  ...
  const names={panel:["Tus actividades","Días, lugares y entregas publicadas por Rhino."],detail:["Detalle de actividad","Información publicada para Aunor."],acordado:["Contrato","Los servicios previstos y el trabajo que se relaciona con ellos."],calendar:["Calendario anual","Las jornadas publicadas, sin perder ninguna actividad coincidente."],replacement:["Reemplazo documentado","Original, sustituto, motivo y evidencia conservados."]};
  return <main className={s.page}>{demo&&<div className={s.demo}><strong>DEMOSTRACIÓN AISLADA · EJEMPLOS FICTICIOS</strong><p>No son datos contractuales reales. Los cambios de esta demostración viven en el servidor local y se reinician al detenerlo.</p></div>}
    ...
    {scene==="panel"&&<><div className={s.grid}>{visible.map(a=><AunorActivityCard key={a.id} a={a} w={w}/>)}</div>{!visible.length&&<p className={s.empty}>No hay actividades publicadas que coincidan con esta búsqueda.</p>}<p className={s.footnote}>“Por relacionar” no oculta el trabajo: solo indica que falta su referencia en Contrato.</p></>}
    {scene==="detail"&&<ActivityDetail key={id} w={w} id={id} mutate={mutate} pending={pending}/>}
