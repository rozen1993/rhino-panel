...
function DetailPanel({ item, close }: { item: SimulatedActivity; close?: () => void }) {
  ...
  return <aside aria-label="Detalle de actividad" className="h-full overflow-y-auto bg-panel p-5 shadow-2xl xl:p-6 xl:shadow-none">
    ...
    <div className="pr-12"><StatusPill status={item.status} /><p className="mt-5 text-[0.625rem] font-extrabold uppercase tracking-[0.16em] text-cyan">{item.type}{item.origin === "burson" ? " · Burson" : ""}</p><h2 className="section-title mt-1 text-xl leading-tight">{item.title}</h2></div>
    ...
    <dl className="mt-5 grid grid-cols-[6.5rem_1fr] gap-3 border-y border-line py-5 text-sm"><dt className="font-extrabold">Responsable</dt><dd>{item.responsible}</dd><dt className="font-extrabold">Fechas</dt><dd>{formatActivityDates(item)}</dd><dt className="font-extrabold">Origen</dt><dd>{item.origin === "burson" ? "Burson" : "Operario"}</dd></dl>
...
function MiniMonth({ year, month, items, onSelect }: { year: number; month: number; items: SimulatedActivity[]; onSelect: (item: SimulatedActivity) => void }) {
  const first = new Date(year, month, 1);
  const days = new Date(year, month + 1, 0).getDate();
  const offset = (first.getDay() + 6) % 7;
  const indexed = useMemo(() => items.map((item) => ({ item, dates: datesFor(item) })), [items]);
  return <section className="rounded-[8px] border border-line bg-panel p-2.5 shadow-[0_3px_10px_rgba(3,29,54,0.035)]">
    <h3 className="text-center text-[0.6875rem] font-extrabold tracking-[0.08em] text-ink">{monthNames[month]}</h3>
...
export function AnnualCalendar() {
  ...
  return <div className="overflow-hidden rounded-[10px] border border-line bg-panel md:grid md:grid-cols-[minmax(0,1fr)_42%] xl:grid-cols-[minmax(0,1fr)_310px]">
    <section className="min-w-0 bg-paper p-3 md:p-5">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-line pb-4"><div><p className="text-[0.625rem] font-extrabold uppercase tracking-[0.2em] text-cyan">Archivo operativo</p><h1 className="section-title mt-1 text-2xl md:text-3xl">Histórico {year}</h1><p className="mt-1 text-sm text-ink-muted">Todas las actividades del año</p></div><div className="flex items-center overflow-hidden rounded-full border border-line bg-panel shadow-sm"><button aria-label="Año anterior" className="min-h-11 min-w-11 text-xl hover:bg-panel-secondary" onClick={() => setYear((value) => value - 1)} type="button">‹</button><strong className="border-x border-line px-5 py-3 tabular-nums">{year}</strong><button aria-label="Año siguiente" className="min-h-11 min-w-11 text-xl hover:bg-panel-secondary" onClick={() => setYear((value) => value + 1)} type="button">›</button></div></header>
      <div aria-label="Leyenda de tipos" className="mt-4 flex flex-wrap gap-x-5 gap-y-2 rounded-[8px] border border-line bg-panel p-3 text-xs">{Object.entries(colors).map(([type, value]) => <span className="flex items-center gap-2" key={type}><i className={`size-2.5 rounded-full ${value.marker}`} />{type}</span>)}</div>
      <div className="mt-4 grid gap-2 lg:grid-cols-2 xl:grid-cols-4">{monthNames.map((_, month) => <MiniMonth items={visible} key={month} month={month} onSelect={select} year={year} />)}</div>
    </section>
    ...
    {overlay && selected && <div className="fixed inset-0 z-[60] bg-night/45 md:hidden" onClick={() => setOverlay(false)}><div className="absolute inset-x-0 bottom-0 max-h-[78vh] overflow-hidden rounded-t-[18px]" onClick={(event) => event.stopPropagation()}><span className="absolute left-1/2 top-2 z-10 h-1 w-20 -translate-x-1/2 rounded-full bg-status-gray" /><DetailPanel close={() => setOverlay(false)} item={selected} /></div></div>}
  </div>;
}
