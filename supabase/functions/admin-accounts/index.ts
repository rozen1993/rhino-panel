...
function usernameEmail(username: string) {
  const configuredDomain = Deno.env.get("SISTEMA_R_USERNAME_DOMAIN")
    ?.trim()
    .toLowerCase();
  const domain = configuredDomain || "auth.sistema-r.invalid";
  if (!validUsernameDomain(domain)) {
